...
export default function ClientDisputes() {
  ...
  const fetchDisputes = async () => {
    try {
      ...
      const response = await api.get('/disputes/client/my-disputes');
  ...
  const fetchMissions = async () => {
    try {
      ...
      console.log('Client missions:', response.data);
      // Filtrer les missions où un automob a été assigné
      const eligibleMissions = response.data.filter(m => 
        m.assigned_automob_id && m.status && ['accepted', 'in_progress', 'completed', 'cancelled', 'en_cours', 'termine', 'annule'].includes(m.status)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/disputes/client/create', data, {
  ...
  const getDecisionBadge = (decision) => {
    ...
    const variants = {
      automob_wins: { variant: 'destructive', label: 'Auto-entrepreneur a gagné' },
      client_wins: { variant: 'success', label: 'Vous avez gagné' },
  ...
  return (
    <DashboardLayout
      title="Mes Litiges"
      description="Gérez vos litiges avec les auto-entrepreneurs"
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <CardDescription>
                        ...
                        <a 
                          href={`/public/automob/${encodeURIComponent(`${dispute.automob_first_name}-${dispute.automob_last_name}`.toLowerCase())}`}
                          className="text-primary hover:underline"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {dispute.automob_first_name} {dispute.automob_last_name}
                        </a>
                      </CardDescription>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => navigate(`/client/disputes/${dispute.id}#messages`)}
                      >
                  ...
                  <SelectContent>
                    {missions.map(mission => (
                      <SelectItem key={mission.id} value={mission.id.toString()}>
                        {mission.title} - {mission.automob_first_name} {mission.automob_last_name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
