...
import { MissionsChart, RevenueChart, ProfileViewsChart } from '@/components/dashboard/MissionsChart';
import { 
  generateWeeklyMissionsData, 
  generateMonthlyMissionsData,
  generateWeeklyRevenueData,
  generateMonthlyRevenueData,
  generateWeeklyProfileViewsData,
  generateMonthlyProfileViewsData,
...
const DashboardAutomob = () => {
  ...
  const [chartPeriod, setChartPeriod] = useState('week'); // 'week' ou 'month'
  const [missionsChartData, setMissionsChartData] = useState([]);
  const [revenueChartData, setRevenueChartData] = useState([]);
  const [profileViewsChartData, setProfileViewsChartData] = useState([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        api.get('/missions/my-applications').catch(() => ({ data: [] })),
        api.get('/timesheets/my-missions').catch(() => ({ data: [] })),
        api.get('/wallet/my-wallet').catch(() => ({ data: { balance: 0 } })),
        api.get('/disputes/automob/my-disputes').catch(() => ({ data: [] })),
        api.get('/notifications').catch(() => ({ data: [] })),
        api.get('/automob/my-reviews').catch(() => ({ data: { reviews: [], averageRating: 0 } }))
      ]);
      ...
      // Calculer la note moyenne correctement
      ...
      setStats({
        ...
        missionsAccepted: missions.filter(m => m.status === 'accepted' || m.status === 'accepte').length,
      ...
      updateChartData(completedMissions, user?.profile?.profile_views || 0);
      ...
      // Prochaine mission acceptée
      const acceptedMissions = missions.filter(m => 
        m.status === 'accepted' && new Date(m.start_date) > new Date()
      ).sort((a, b) => new Date(a.start_date) - new Date(b.start_date));
      
      if (acceptedMissions.length > 0) {
        setNextMission(acceptedMissions[0]);
      }
  ...
  const updateChartData = (completedMissions, profileViews) => {
    if (chartPeriod === 'week') {
      setMissionsChartData(generateWeeklyMissionsData(completedMissions));
      setRevenueChartData(generateWeeklyRevenueData(completedMissions));
      setProfileViewsChartData(generateWeeklyProfileViewsData(profileViews));
    } else {
      setMissionsChartData(generateMonthlyMissionsData(completedMissions));
      setRevenueChartData(generateMonthlyRevenueData(completedMissions));
      setProfileViewsChartData(generateMonthlyProfileViewsData(profileViews));
    }
  };

  useEffect(() => {
    if (stats.missionsCompleted > 0 || stats.profileViews > 0) {
      // Recharger les données de graphiques quand la période change
      fetchDashboardData();
    }
  }, [chartPeriod]);
  ...
  return (
    <DashboardLayout
      ...
      description={user?.email ? `Bienvenue, ${displayName()}` : 'Espace auto-entrepreneur'}
    ...
    >
      <section className="space-y-6">
        ...
        <div>
          <h3 className="text-lg font-semibold mb-4">Mes Candidatures</h3>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-3 sm:p-4 sm:pb-2">
                <CardTitle className="text-sm font-medium">Total Postulées</CardTitle>
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent className="p-3 pt-0 sm:p-4 sm:pt-0">
                <div className="text-2xl font-bold">{stats.missionsAccepted}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="p-3 pt-0 sm:p-4 sm:pt-0">
                <div className="text-2xl font-bold text-green-600">
                  {parseFloat(stats.walletBalance).toFixed(2)} €
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button 
                  variant="ghost" 
                  size="sm"
                  onClick={() => navigate('/automob/disputes')}
                >
                  Voir tout
                </Button>
              </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button 
                  variant="ghost" 
                  size="sm"
                  onClick={() => navigate('/automob/notifications')}
                >
                  Voir tout
                </Button>
              </div>
...
export default DashboardAutomob;
