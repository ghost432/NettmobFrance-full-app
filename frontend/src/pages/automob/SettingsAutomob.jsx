...
import { Lock, Bell, Mail, Eye, EyeOff, History, Timer, CreditCard, User, MapPin, Phone } from 'lucide-react';
import { toast } from '@/components/ui/toast';
import api from '@/lib/api';
import { usersAPI } from '@/lib/api';
import { cn } from '@/lib/utils';
import { useDocumentTitle } from '@/hooks/useDocumentTitle';
import { enablePushNotifications, disablePushNotifications, sendTestNotification } from '@/services/fcmService';

const SettingsAutomob = () => {
  ...
  const sections = useMemo(() => ([
    {
      id: 'security',
      label: 'Sécurité',
      description: 'Gérez votre mot de passe et sécurisez votre compte.',
      icon: Lock,
    },
    {
      id: 'billing',
    ...
    },
    {
      id: 'notifications',
      label: 'Notifications',
      description: 'Activez les alertes importantes pour vos missions.',
      icon: Bell,
    },
    {
      id: 'history',
      label: 'Historique des connexions',
      description: 'Consultez vos dernières connexions et la durée cumulée.',
      icon: History,
    },
  ]), []);
  ...
  const handleNotificationToggle = async (type, value) => {
    ...
    try {
      // Si c'est Web Push, gérer la souscription Firebase
      if (type === 'webPushEnabled' && value) {
        console.log('[Toggle] Activation notifications push Firebase...');
        ...
        console.log('[Toggle] Désactivation notifications push Firebase...');
  ...
  const BillingCard = () => {
    const handleBillingSubmit = async (e) => {
      ...
      try {
        const { data } = await api.put('/users/profile/automob', billingInfo);
        const updatedUser = {
          ...user,
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Ces informations seront utilisées pour la facturation</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <div className="grid gap-6 lg:grid-cols-[260px,1fr]">
          <Card className="h-fit">
            <CardHeader>
              ...
              <CardDescription>Configurez votre compte auto-entrepreneur</CardDescription>
            </CardHeader>
...
export default SettingsAutomob;
